...
export function useSelectionParam(
...
): [string[], (id: string, include?: boolean) => void, (next: string[]) => void] {
  ...
  const selected = useMemo(() => {
    ...
    const filtered = allowed ? unique.filter((id) => allowed.includes(id)) : unique;
    if (!orderMap) {
      return filtered.slice().sort();
    }
    return filtered.slice().sort((a, b) => (orderMap.get(a) ?? 0) - (orderMap.get(b) ?? 0));
  }, [searchParams, key, defaults, allowed, orderMap]);

  const apply = useCallback(
    (nextValues: string[]) => {
      const cleaned = allowed ? nextValues.filter((id) => allowed.includes(id)) : nextValues;
